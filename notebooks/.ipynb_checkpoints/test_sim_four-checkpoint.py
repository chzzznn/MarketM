...
def simulate_round():
    ...
    event_name = run_simulation_round(st.session_state.assets, st.session_state.orderbook, st.session_state.portfolios)
...
if st.session_state.current_sentiment:
    st.write(f"🧠 Sentiment: **{st.session_state.current_sentiment}**")

if st.button("Simulate Next Round"):
    simulate_round()
...
portfolio_df = pd.DataFrame(st.session_state.portfolio_value_history)
st.line_chart(portfolio_df)

st.subheader("Sentiment & Event Log")
